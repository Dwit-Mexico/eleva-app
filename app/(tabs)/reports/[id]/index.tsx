...
import { formatDate, formatVisit, parseDay, relativeTime } from '@/lib/relativeTime';
...
function Body({ r, onCancel, cancelError }: { r: Request; onCancel: () => void; cancelError?: string }) {
  ...
  const lang = currentLanguage();
  const created = parseDay(r.createdAt);
...
function MessagesCard({ requestId, onPress }: { requestId: number; onPress: () => void }) {
  ...
  const last = thread?.lastDeleted
    ? t('chat.deleted')
    : thread?.lastText || (thread?.lastPhoto ? t('chat.photo') : '');
  const sub = last
    ? thread?.lastMine
