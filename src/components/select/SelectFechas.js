...
function SelectArea({ fechas, value, onChange }) {
  ...
  const selected = fechas?.find(item => item.id === value)
      ...
      >
        <View style={styles.selectContent}>
          <Text style={[styles.selectText, { color: selected ? '#000' : '#777' }]}>
            {selected ? selected.name : i18n.t('select.selectDate')}
          </Text>
      ...
      <Modal visible={isVisible} onRequestClose={cancelar} animationType="fade" transparent statusBarTranslucent>
        <SafeAreaView style={styles.safeArea} edges={['top', 'left', 'right']}>
          <View style={styles.container}>
            ...
            <View style={[styles.bottomSheet, { paddingBottom: Math.max(insets.bottom, 30) }]}>
              <Text style={styles.title}>{i18n.t('select.date')}</Text>

              <FlatList
                data={fechas || []}
                keyExtractor={item => String(item.id)}
                showsVerticalScrollIndicator={false}
                ListEmptyComponent={<Text style={styles.emptyText}>{i18n.t('select.empty')}</Text>}
                renderItem={({ item }) => {
